...
void gpio_callback(uint gpio, uint32_t events) {
    ...
    if (gpio == PCLK){
        if (saveImage && startImage && startCollect) {
            vsCount++;
           // read each data bit separately (because D0..D7 are not GP0..GP7)
            uint8_t byte = 0;
            byte |= (gpio_get(D0) & 0x1) << 0;  // D0 → LSB
            byte |= (gpio_get(D1) & 0x1) << 1;  // D1 → bit1
            byte |= (gpio_get(D2) & 0x1) << 2;
            byte |= (gpio_get(D3) & 0x1) << 3;
            byte |= (gpio_get(D4) & 0x1) << 4;
            byte |= (gpio_get(D5) & 0x1) << 5;
            byte |= (gpio_get(D6) & 0x1) << 6;
            byte |= (gpio_get(D7) & 0x1) << 7;

            cameraData[rawIndex++] = byte;
...
// =======================================
// Implementation of findCOM2D( ). Place this anywhere below your globals.
// =======================================
void findCOM2D(int *outX, int *outY) {
    // We assume IMAGESIZEX = width, IMAGESIZEY = height, and
    // picture.r[], picture.g[], picture.b[] each have (W*H) entries.
    long long sumMass = 0;
    long long sumX    = 0;
    long long sumY    = 0;
    ...
    for (int y = 0; y < IMAGESIZEY; y++) {
        for (int x = 0; x < IMAGESIZEX; x++) {
            ...
            // brightness = R+G+B:
            int mass = picture.r[idx] + picture.g[idx] + picture.b[idx];
            sumMass += mass;
            sumX    += (long long)mass * x;
            sumY    += (long long)mass * y;
        }
    }

    if (sumMass > 0) {
        // integer division is fine; you can round if you want sub-pixel
        *outX = (int)(sumX / sumMass);
        *outY = (int)(sumY / sumMass);
    } else {
        // If the image was completely black (sumMass=0), just pick center:
        *outX = IMAGESIZEX / 2;
        *outY = IMAGESIZEY / 2;
    }
}
...
// ──────────────────────────────────────────────────────────────────────────
// Implementation of findCOMWhite( ). Place this anywhere below your globals.
// ──────────────────────────────────────────────────────────────────────────
void findCOMWhite(int *outX, int *outY) {
    // 1) Compute global average brightness:
    long long sumBrightAll = 0;
    int totalPixels = IMAGESIZEX * IMAGESIZEY;
    for (int i = 0; i < totalPixels; i++) {
        sumBrightAll +=
            (long long)picture.r[i]
          + (long long)picture.g[i]
          + (long long)picture.b[i];
    }
    // avgBrightness = (R+G+B) average over all pixels
    int avgBrightness = (int)(sumBrightAll / totalPixels);

    // 2) Now sum up only those pixels whose brightness >= avgBrightness
    long long sumX = 0;
    long long sumY = 0;
    int      whiteCount = 0;
    ...
    for (int y = 0; y < IMAGESIZEY; y++) {
        for (int x = 0; x < IMAGESIZEX; x++) {
            int idx = y * IMAGESIZEX + x;
            int bright = picture.r[idx] + picture.g[idx] + picture.b[idx];
            if (bright >= avgBrightness) {
                // treat as “white” pixel
                sumX += x;
                sumY += y;
                whiteCount++;
            }
    ...
    if (whiteCount > 0) {
        // The integer COM of all white pixels:
        *outX = (int)(sumX / whiteCount);
        *outY = (int)(sumY / whiteCount);
    } else {
        // If no white pixels, just pick center of image:
        *outX = IMAGESIZEX / 2;
        *outY = IMAGESIZEY / 2;
    }
}
